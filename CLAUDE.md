...
- **Entry**: `src/index.ts`
- **Routes** (`src/routes/`): REST endpoints for auth, sessions (fork, send-to-user), files, skills, settings, health, MCP, users
...
- **Session rebinding**: Pi sessions persist in-memory across WebSocket reconnects via `rebindSocket()`. Turn-scoped buffers (`assistantBuffer`, `turnImagePaths`, `turnToolCalls`, `pendingAskUser`) live on the `PiSessionHandle.turnState` object (not in the WebSocket handler closure) so they survive disconnects. Accumulation and DB persistence happen in the Pi `session.subscribe()` callback in `pi-session.ts` (which fires unconditionally, even when no socket is connected). On rebind, `rebindSocket()` replays all accumulated tool calls, assistant text, and any pending `ask_user` question to the new socket so the client catches up on anything it missed — including re-rendering the ask_user card if the agent is waiting for user input.
...
- **Tool call persistence**: Tool calls (bash output, file reads/edits, etc.) are persisted in the `tool_calls` TEXT column of the messages table, following the same pattern as inline images. The flow:
  - Server-side (`pi-session.ts`): the `session.subscribe()` callback accumulates tool call records (`{ tool, args, result, isError }`) into `turnState.turnToolCalls` as Pi events fire. Results are truncated at 4000 chars. On `agent_end`, the buffer is serialized to JSON and attached to the assistant message via `addMessage()`. If the LLM produced no text (only tool-use responses), tool calls are attached to the last assistant message via `setLastMessageToolCalls()` (preserving correct timeline ordering). If no assistant message exists at all, a new message with empty content is created as a fallback — this prevents silent data loss when the UPDATE would match 0 rows. Because accumulation happens in the subscribe callback (not a WebSocket send interceptor), tool calls are captured even when the user is disconnected (e.g. viewing a different session).
  ...
  - Fork/send-to: `forkSession()` copies `tool_calls` alongside `images` when duplicating messages. Accepts optional `targetUserId` for cross-user sends.
  - **Smart truncation** (`truncateToolResult` in `pi-session.ts`): Tool results with a `details` object (Pi SDK `ToolResult` format, e.g. subagent results) are truncated field-by-field — `content[].text` and nested `details.toolCalls[].result` values are individually capped at 4000 chars — preserving the object structure. Plain objects without `details` fall back to JSON stringify + slice.
- **Send to user** (`POST /sessions/:id/send-to`): Forks a session to a different user. The new session is owned by the target user and named `"Session Name (from SenderName)"`. Copies all messages (with images, tool_calls) and workspace files. The sender must own the source session; sending to self is rejected (use fork instead). `GET /api/users` returns all users except the caller (id, username, displayName) for the user picker. Frontend: Send icon (paper plane) in session sidebar; `SendToUserDialog` component (portaled to `document.body`) with user list, selection highlight, and success feedback.
- **Live session previews**: The sidebar shows live preview text under each session name. For running sessions, the server computes a preview from `turnState` (priority: pending `ask_user` question → last ~100 chars of assistant buffer → name of currently running tool → "Working..."). The client polls `GET /sessions/running` every 3s, which returns `{ sessionIds, previews }`. Running session previews display in italic primary-colored text. Completed sessions show `lastMessage` from the DB, updated by also polling the session list every 3s. The `runningPreviews` Map in the session store maps session IDs to preview strings.
...
- **Subagent delegation** (`tools/subagent.ts`): The parent Pi agent can spawn child agents via the `subagent` tool. Each child gets a fresh context window, an in-process `createAgentSession()` with `SessionManager.inMemory()`, and the same sandbox integration as the parent (via `_baseToolsOverride` patching). Key design:
  - **Default tools**: `read`, `write`, `edit`, and `bash`. Parent can opt-in `web_search`, `web_fetch` via the `tools` param.
